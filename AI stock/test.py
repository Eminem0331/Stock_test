...
sql = '''
select
a.id,a.ts_code,a.ann_date,a.end_date,a.roe_dt,current_ratio,quick_ratio,a.factor_style,a.Update_frequency
...
and a.end_date>='2007-01-01' and a.end_date<='2007-12-31'
;
'''
sql2 = '''
select
distinct end_date
from stock_data.fina_factor a
where a.end_date>='2007-01-01' and a.end_date<='2007-12-31'
;
'''
...
for n,date in enumerate(date_list[0:-1]):
     print(date)
     # rrr=[]
     def  sing(index,date1,date2):
        rrr = []
        df = data[data['end_date'] == date1]
        print(index)
        #计算IC
        stock=df['ts_code']
        re=[]
        for m in stock:
            re.append(close_sql(m, date1, date2))
        ...
        IC=get_ic(re,index,date1)

        #计算分位数换手率与换手率
        stock1=Stock_Quantile_qe(data,index,0.2,date1)
        stock2=Stock_Quantile_qe(data,index,0.2,date2)
        turnover1=turnover(stock1,stock2)
        stock3=Stock_Quantile_le(data,index,0.8,date1)
        stock4=Stock_Quantile_le(data,index,0.8,date2)
        turnover2=turnover(stock3,stock4)
        re1=[]
        for j in stock1:
             # print(j)
            ddd=close_sql(j,date1,date2)
            re1.append(ddd)
        re2=[]
        for k in stock3:
                   # print(k)
            dddd=close_sql(k,date1,date2)
            re2.append(dddd)
        ...
        rrr.append([index,date1,future_return1,future_return2,turnover1,turnover2,IC])
        rrr = pd.DataFrame(rrr,
                           columns=['factor_name', 'end_date', 'Minimum_quantile_return', 'Maximum_quantile_return',
                                    'Minimum_quantile_turnover', 'Maximum_quantile_turnover', 'IC'])
        print(rrr)

     def  main(index_list,date1,date2):
        work=[]
        for index in index_list:
            work.append(threading.Thread(target=sing, args=(index,date1,date2)))
        for i in work:
            i.start()
     ...
     main(index_list,date,date_list[n+1])
